...
def sort_conts_x(section_regs):
	'''sort the rectangles in each line by x'''
	print 'before sort by x' 
	for line in range(len(section_regs)):
		for reg in range(len(section_regs[line])):
			print 'reg[', line, '][', reg, ']', section_regs[line][reg]
	re_section_regs = []
	for current_section_regs in section_regs:
		current_section_regs = sorted(current_section_regs, key = itemgetter(0))
		re_section_regs.append(current_section_regs)


	return re_section_regs
